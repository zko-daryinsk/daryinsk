...
function resetSearch() {
    ...
    const tabsRow = document.querySelector(".categories-control-row"); if (tabsRow) tabsRow.style.display = "flex";
    const firstTag = document.querySelector(".card-tag");
    if (firstTag) {
        const curLabel = firstTag.innerText;
        for (let key in categoryTranslations) { if (categoryTranslations[key][window.app.currentLang] === curLabel || key === curLabel) { window.app.activeCategory = key; break; } }
    }
    renderCards();
