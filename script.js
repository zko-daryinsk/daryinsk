// ==========================================
// БЛОК 1: КОНФИГ И СЛОВАРИ (ЧАСТЬ А)
// ==========================================
...
// ==========================================
// БЛОК 1: КОНФИГ И СЛОВАРИ (ЧАСТЬ Б)
// ==========================================
const dict = {
    ru: {
...
// ==========================================
// БЛОК 2: МОТОР ЯДРА (ЗАГРУЗКА ДАННЫХ)
// ==========================================
function initApp() {
    restoreViewMode();
    loadVillages();
...
// ==========================================
// БЛОК 3: УТИЛИТЫ И МАСКИ НОМЕРОВ С КОНЦА
// ==========================================
...
function formatPhoneNumber(pure10Digits) {
    ...
    if (pure10Digits.startsWith("71131")) {
        return "+7 (" + pure10Digits.substring(0, 5) + ") " + pure10Digits.substring(5, 6) + "-" + pure10Digits.substring(6, 8) + "-" + pure10Digits.substring(8, 10);
    }
...
function renderCards() {
    ...
    grouped.forEach(function(item) {
        ...
        item.phones.forEach(function(pObj) {
            ...
            if (pure10.length !== 10) { html += "<div class='card-phone-line' style='color:#dc2626;'>📱 " + pObj.num + "</div>"; }
            else {
...
// ==========================================
// БЛОК 4: ИНТЕРФЕЙС И СКВОЗНОЙ СВАЙП 1 СМ
// ==========================================
...
window.addEventListener("touchend", function(e) {
    ...
    if (Math.abs(diffX) > 100 && Math.abs(diffY) < 45) {
        ...
        if (categories.length > 0) {
            ...
            window.app.activeCategory = categories[idx]; window.app.activeLetter = ""; 
...
function sendNewContact() { var sv = document.getElementById("formCatSelect").value; var cat = (sv === "CUSTOM_OPTION") ? document.getElementById("formCatCustom").value.trim() : sv; var rp = document.getElementById("formPhone").value.trim(); var t = document.getElementById("formName").value.trim(); var pd = getClean10Digits(rp); if (!cat || !pd || !t) { alert(dict[window.app.currentLang].mAlert); return; } var hasWA = document.getElementById("formHasWA").checked; var line = cat.toUpperCase() + " | " + t + " | " + pd + " | " + document.getElementById("formFio").value.trim() + " | " + document.getElementById("formDesc").value.trim() + " | " + document.getElementById("formLoc").value.trim() + " | " + (hasWA ? "$" : "NO_WA"); var btn = document.getElementById("btnSubmitForm"); btn.disabled = true; btn.innerText = "..."; window.location.href = "whatsapp://send?phone=" + adminPhone + "&text=" + encodeURIComponent("Здравствуйте! Прошу добавить контакт.\n\n" + line); setTimeout(function() { btn.disabled = false; btn.innerText = dict[window.app.currentLang].mSubmit; closeModal("addModal"); }, 2000); }
...
function resetSearch() {
    ...
    const firstTag = document.querySelector(".card-tag");
    if (firstTag) {
        const curL = firstTag.innerText;
